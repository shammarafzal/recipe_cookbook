          ...
          return FutureBuilder(
              ...
              builder: (context, AsyncSnapshot snapshot) {
                ...
                if (!snapshot.hasData) {
                  ...
                  List.from(snapshot.data[i]['ingredients']);
                  List<String> directions =
                  List.from(snapshot.data[i]['directions']);
                  String title = snapshot.data[i]['title'].toString();
                  num rating = snapshot.data[i]['rating'];
                  String category =
                  snapshot.data[i]['category'].toString();
                  String image = snapshot.data[i]['image'].toString();
                  String id = snapshot.data[i]['id'].toString();
                  String time = snapshot.data[i]['time'].toString();
                  String calorie = snapshot.data[i]['calorie'].toString();
                  return InkWell(
                    onTap: () {
                      Navigator.of(context).push(PageRouteBuilder(
  ...
  Widget build(BuildContext context) {
    return PageView.builder(
        ...
        itemBuilder: (context, i) {
          ...
          String imageUrl = list[i]['imageUrl'].toString();
          String id = list[i]['id'].toString();
          String description = list[i]['desc1'].toString();
          String price = list[i]['price'].toString();
          String hours = list[i]['time'].toString();
          String date = list[i]['date'].toString();
          String location = list[i]['place'].toString();
          String description2 = list[i]['desc2'].toString();
          String description3 = list[i]['desc3'].toString();
          return InkWell(
            onTap: () {},
            child: Container(
              width: MediaQuery.of(context).size.width,
...
class cardNear extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListView.builder(
      ...
      itemBuilder: (context, i) {
        List<String> ingredients = List.from(list[i]['ingredients']);
        List<String> directions = List.from(list[i]['directions']);
        String title = list[i]['title'].toString();
        num rating = list[i]['rating'];
        String category = list[i]['category'].toString();
        String image = list[i]['image'].toString();
        String id = list[i]['id'].toString();
        String time = list[i]['time'].toString();
        String calorie = list[i]['calorie'].toString();
